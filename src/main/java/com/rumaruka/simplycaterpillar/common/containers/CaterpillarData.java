package com.rumaruka.simplycaterpillar.common.containers;

import com.rumaruka.simplycaterpillar.Caterpillar;
import com.rumaruka.simplycaterpillar.Const;
import com.rumaruka.simplycaterpillar.common.parts.PartsMovement;
import com.rumaruka.simplycaterpillar.common.parts.PartsTabs;
...
public class CaterpillarData implements Cloneable {
    ...
    public void updateScroll(Container myDrillHeads) {
        ...
        Const.printDebug( this.storage.added + "," + Middle);
        ...
        else if (MiddleNewStorage < MiddleOldStorage)
        {
            ...
            for (int i = 0; i < this.inventory.length; i++) {
                ...
                }else if (i >= MiddleNewStorage + offset && i < MiddleOldStorage + offset)
                {
                    if (this.inventory[i] != null)
                    {
                        Const.dropItem(worldIn, this.pos, this.inventory[i]);
                    }
    ...
    public static CaterpillarData readCaterpiller(NBTTagCompound NBTconCat)
    {
        ...
        CaterpillarData newCatp = new CaterpillarData(posP, key);
        newCatp.tabs.selected = Caterpillar.GuiTabs.values()[NBTconCat.getInteger("selectedtab")];
        ...
        newCatp.inventory = Const.MainNBT.readItemStacks(NBTconCat);
        ...
        if (newCatp.inventory.length < getMaxSize() + newCatp.storage.added || newCatp.inventory.length > getMaxSize() + newCatp.storage.added){
            ...
            Const.printDebug("Inventory length was wrong had to resize: " + newCatp.inventory.length + "," + tmpY.length);
    ...
    public NBTTagCompound writeNBTCaterpillar() {
        NBTTagCompound NBTconCat = Const.MainNBT.writeItemStacks(this.inventory);
